<?php
    ...
    /*
     * at this point we fake a bit.the Access Manager sends us the username
    ...
    if (isset($_SERVER['HTTP_X_USERNAME']) && $_SERVER['HTTP_X_USERNAME'] != '') {
       ...
       #error_log('Novell username: ' . $_SERVER['HTTP_X_USERNAME']);
       ...
       if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'submitlogin') {
          ...
          error_log('Access Manager login by: ' . $_SERVER['HTTP_X_USERNAME']);
       }
    };
 
    # include the file extensions/NovellLoginFix.php in index.php after mediawiki.initialize() to fix login issues that cannot be fixed here


    //use Access Manager for login
    class NovellAuthenticationPlugin extends AuthPlugin {
        /**
    ...
    $wgHooks['UserLogoutComplete'][] = 'NovellLogout';
